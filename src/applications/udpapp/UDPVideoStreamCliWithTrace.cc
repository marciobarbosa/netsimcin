...
void UDPVideoStreamCliWithTrace::initialize()
{
	...
	this->trace_out = fopen("trace_out", "w");
...
void UDPVideoStreamCliWithTrace::finish()
{
	...
	recordScalar("decodable frame rate (Q)", numFramesReceived/double(numFramesReceived+numFramesDiscarded));
		
	fclose(this->trace_out);
}

int cont_teste = 0; // My CODE
...
/*
	...
	{
	    case IDR:
	        fprintf(this->trace_out, "%d\t", pkt->getFrameNumber());
	        fprintf(this->trace_out, "%s\n", "IDR");
	        break;
	    case I:
	        fprintf(this->trace_out, "%d\t", pkt->getFrameNumber());
	        fprintf(this->trace_out, "%s\n", "I");
...
*/
    ...
    if (warmupFinished == false)
    {
    	...
    	{
    		if (frameType == I || frameType == IDR) // My CODE
    		{
